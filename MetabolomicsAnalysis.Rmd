...
```{r Muma, warning= FALSE, message=FALSE}
#These functions correspond to the muma package which can be found on GitHub (https://github.com/cran/muma/tree/master/R)
#Since the package has not been updated in a while it can not be installed with the newest bioconductor version 3.1 and R version 4.2.0
#Therefore, we have extracted the two main function of the package we use for the outlier detection
#In explore.data and Plot.pca.score we added the @parameter "path"

explore.data <- function (file, path, scaling, scal = TRUE, normalize = TRUE, imputation = FALSE, imput) {
    ...
    dirout = paste(getwd(), path, "/Preprocessing_Data_", scaling, 
        "/", sep = "")
    dir.create(dirout)
    if (imputation) {
        y = x.x
        ...
        pwdi = paste(getwd(), path, "/Preprocessing_Data_", scaling, 
	...
	pwd.c = paste(getwd(), path, "/Preprocessing_Data_", scaling, "/CorrectedTable.csv", 
        sep = "")
    ...
    if (scal) {
        if (scaling == "Pareto" | scaling == "pareto" | scaling == 
            "P" | scaling == "p") {
            pwd.n = paste(getwd(), path, "/Preprocessing_Data_", scaling, 
    ...
    pwd.scal = paste(getwd(), path, "/Preprocessing_Data_", scaling, 
        "/ProcessedTable.csv", sep = "")
    ...
    p.f <- cbind(p.i, p.z)
    dirout.pca = paste(getwd(),path, "/PCA_Data_", scaling, "/", sep = "")
    ...
    K = paste(getwd(), path, "/Preprocessing_Data_", scaling, "/class.csv", 
        sep = "")
    write.csv(k, K)
    ...
    dirout.g = paste(getwd(), path, "/Groups", sep = "")
    dir.create(dirout.g)
    for (i in 1:nrow(g)) {
        vuota <- c()
...
t2 = c(t1[,1])
    ...
    rownames(all.F) = nam
    allFpwd = paste(getwd(),path, "/PCA_Data_", scaling, "/PCs_Fstatistic.csv", 
        sep = "")
    ...
    rankFpwd = paste(getwd(), path, "/PCA_Data_", scaling, "/PCs_ranked_Fpvalue.csv", 
        sep = "")
...
Plot.pca.score <- function(path, pcx,pcy,scaling) {
 score = paste(getwd(),  path,"/PCA_Data_", scaling, "/PCA_ScoreMatrix.csv", sep="")
 ppppp = paste(getwd(), path,"/PCA_Data_", scaling, "/PCA_P", sep="")
 Score <- read.csv(score, sep=",", header=TRUE)
 Score.x <- Score[,2:ncol(Score)]
 rownames(Score.x) <- Score[,1]
 pwdK = paste(getwd(),path, "/Preprocessing_Data_", scaling, "/class.csv", sep="")
 k = read.csv(pwdK)
 ...
 D <- paste(getwd(),path, "/PCA_Data_", scaling, "/ScorePlot_PC",pcx,"vsPC",pcy,".pdf", sep="")
 dev.copy2pdf(file=D)
}
...
#Load the data:
##Human:
data_H <- read.csv("Input/Amin_human_final.csv", check.names=FALSE)%>%
...
data_H <-data_H[,c(1,3:4,2,5:6,8:235)]
data_H[, c(6:8)] <- sapply(data_H[, c(6:8)], as.numeric)

##Mouse:
data_M <- read.csv("Input/Amin_mouse_final.csv", check.names=FALSE)%>%
  separate("timepoint",c("timepoint_min","X"),sep=" ", remove=FALSE)
data_M <-data_M%>%
  unite("Sample",c("Mouse_Number","timepoint"),sep="_", remove=FALSE)%>%
  unite("UniqueID",c("Sample","analytical_replicate"),sep="_", remove=FALSE)
data_M <-data_M[,c(1,3:4,2,7:8, 5:6,10:274)]
data_M[, c(6:273)] <- sapply(data_M[, c(6:273)], as.numeric)
...
```{r, eval=FALSE}
Muma_Intra_H <- H_norm
Muma_Intra_H <- Muma_Intra_H[,c(2,8,10:234)]
...
explore.data("Output/DataTables/Muma_Intra_H.csv",#Data need to be imported as a .csv
             path="/Muma Analysis/Human",
             "Pareto",#scaling is performed
             normalize=FALSE,
             imputation = FALSE)

#Test for Outlier based on geometric distances of each point in the PCA score plot:
Plot.pca.score(path="/Muma Analysis/Human",
               1,#an integer indicating the principal component to be plotted in x
...
dim(dataHeat)
Input_dataHeat <- dataHeat[,10:234]
...
```{r, eval=FALSE}
Muma_Intra_M <- M_norm
Muma_Intra_M <- Muma_Intra_M[,c(2,8,9:273)]
...
explore.data("Output/DataTables/Muma_Intra_M.csv",#Data need to be imported as a .csv
             path="/Muma Analysis/Mouse",
             "Pareto",#scaling is performed
             normalize=FALSE,
             imputation = FALSE)

#Test for Outlier based on geometric distances of each point in the PCA score plot:
Plot.pca.score(path="/Muma Analysis/Mouse",
               1,#an integer indicating the principal component to be plotted in x
               2,#an integer indicating the principal component to be plotted in y
               "Pareto")#scaling previously specified in the function ’explore.data’
```
...
H_0min <-subset(Data_DMA,timepoint == "0 min" ,select=c(8:231))
H_5min <-subset(Data_DMA,timepoint == "5 min" ,select=c(8:231))
H_15min <-subset(Data_DMA,timepoint == "15 min" ,select=c(8:231))
H_60min <-subset(Data_DMA,timepoint == "60 min" ,select=c(8:231))
H_120min <-subset(Data_DMA,timepoint == "120 min" ,select=c(8:231))

#---------------------
## Mean of the replicates:
Data_DMA_Means <- (Data_DMA[,8:231]) %>%
  group_by(Data_DMA$timepoint) %>%
  summarise_all(funs(mean))
#Extract the single conditions we want to use for the comparison
Min0 <-Data_DMA_Means[1,2:225]
Min5 <-Data_DMA_Means[4,2:225]
Min15 <-Data_DMA_Means[3,2:225]
Min60 <-Data_DMA_Means[5,2:225]
Min120 <-Data_DMA_Means[2,2:225]
...
#---------------------
## Single Replicates for each timepoints:
M_0min <-subset(Data_DMA,timepoint == "0 min" ,select=c(7:270))
M_5min <-subset(Data_DMA,timepoint == "5 min" ,select=c(7:270))
M_15min <-subset(Data_DMA,timepoint == "15 min" ,select=c(7:270))
M_60min <-subset(Data_DMA,timepoint == "60 min" ,select=c(7:270))
M_120min <-subset(Data_DMA,timepoint == "120 min" ,select=c(7:270))

#---------------------
## Mean of the replicates:
Data_DMA_Means <- (Data_DMA[,7:270]) %>%
  group_by(Data_DMA$timepoint) %>%
  summarise_all(funs(mean))
#Extract the single conditions we want to use for the comparison
MMin0 <-Data_DMA_Means[1,2:265]
MMin5 <-Data_DMA_Means[4,2:265]
MMin15 <-Data_DMA_Means[3,2:265]
MMin60 <-Data_DMA_Means[5,2:265]
MMin120 <-Data_DMA_Means[2,2:265]
